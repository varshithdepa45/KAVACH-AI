...
export default function WorkbenchPage() {
  ...
  const { data: availableDocuments, source: documentSource } = useLive(
    fetchDocuments,
    documents,
  );
  ...
  const needsReview = findings.filter(
    (f) => !f.verified || f.reviewState === "needs-review",
  );
  ...
  async function analyzeSelectedDocument() {
    if (!selectedDocumentId || documentSource !== "live") return;
    ...
    try {
      ...
      const result = await api.analyzeDocument(
        selectedDocumentId,
        task || `Analyze ${selected?.name ?? "uploaded document"}`,
        "Review this uploaded document and identify important findings with supporting evidence.",
      );
  ...
  return (
    <div>
      <PageTitle eyebrow="Agentic Workspace" title="Workbench">
        <StatusPill
          tone={
            finished ? "verified" : phase === "running" ? "signal" : "muted"
          }
          pulse={phase === "running"}
        >
          {phase === "idle"
              ...
              {availableDocuments.map((d) => (
                <li key={d.id} className="px-3 py-2.5">
                  ...
                  <div className="mt-1.5 flex items-center gap-2 font-mono text-2xs text-verified">
                    <span>OCR {d.ocr === "done" ? "✓" : "·"}</span>
                    <span>EMB {d.embedding === "done" ? "✓" : "·"}</span>
                    <span>IDX {d.indexed === "done" ? "✓" : "·"}</span>
                  </div>
                </li>
              ))}
            </ul>
            <div className="flex items-center gap-1.5 border-t border-line px-3 py-2 font-mono text-2xs text-ink-muted">
              <Lock className="h-3 w-3 text-signal" /> local only · no external
              upload
            </div>
          </Panel>
        </div>
        ...
        <div className="space-y-3">
          ...
          {tab === "pid" ? (
            ...
            <>
              ...
              <Panel>
                <PanelHeader
                  title="What would you like KAVACH to do?"
                  sub="Natural-language task · routed to local agents"
                />
                <div className="p-4">
                  <textarea
                    value={task}
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
                    <label className="block">
                      ...
                      <select
                        ...
                        className="w-full rounded-[3px] border border-line bg-surface-inset px-3 py-2 text-xs text-ink outline-none focus:border-signal/50"
                      >
                        {availableDocuments.map((d) => (
                    ...
                    <button
                      onClick={analyzeSelectedDocument}
                      disabled={
                        analysisState === "running" || documentSource !== "live"
                      }
                  ...
                  {analysisMessage && (
                    <p
                      className={cn(
                        "mt-2 rounded-[3px] border px-3 py-2 font-mono text-2xs",
                        analysisState === "error"
                ...
                <Panel className="grid-bg">
                  <div className="flex flex-col items-center justify-center gap-2 px-6 py-14 text-center">
                    ...
                    <p className="max-w-md text-xs text-ink-muted">
                      Press{" "}
                      <span className="font-mono text-signal">Run KAVACH</span>{" "}
                      to route this task through the security, classification,
                      planning, analysis, retrieval, reasoning, verification and
                      deliverable agents — all locally.
                    </p>
                  </div>
                </Panel>
              )}
                ...
                <>
                  ...
                  <div className="grid gap-3 md:grid-cols-3">
                    ...
                    >
                      ...
                      <div className="mt-1.5 font-mono text-2xs text-ink-muted">
                        vision + reasoning selected
                      </div>
                    </StepCard>
                    <StepCard
                      step="03"
                    ...
                    >
                      <ol className="space-y-0.5">
                        {PLAN.map((p, i) => (
                          <li
                            key={i}
                            className="flex gap-1.5 text-2xs text-ink-muted"
                          >
                    ...
                    <Panel className="flex flex-col">
                      ...
                      <div className="flex-1 overflow-y-auto p-3">
                        <pre className="whitespace-pre-wrap font-mono text-2xs leading-relaxed text-ink-muted">
                          {logLines.join("\n")}
                          {phase === "running" && (
                            <span className="animate-blip text-signal">▊</span>
                          )}
                        </pre>
                      </div>
                    </Panel>
