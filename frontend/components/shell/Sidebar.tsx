...
export function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 flex w-14 flex-col border-r border-line bg-surface/80 backdrop-blur md:w-[236px]">
      {/* Brand */}
      <div className="flex items-center justify-center gap-3 border-b border-line px-2 py-4 md:justify-start md:px-4">
      ...
      {/* Nav */}
      <nav className="flex-1 overflow-y-auto px-1 py-3 md:px-2">
        <p className="eyebrow hidden px-2 pb-2 md:block">Console</p>
            ...
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={cn(
                    "group flex items-center justify-center gap-3 rounded-[3px] border border-transparent px-2.5 py-2 transition-colors md:justify-start",
                    active
                ...
                >
                  ...
                  />
                  <span className="hidden flex-1 text-sm md:block">
                    {item.label}
                  </span>
                  <span
                    className={cn(
                      "hidden font-mono text-2xs md:block",
      ...
      <div className="hidden border-t border-line px-3 py-3 md:block">
        ...
        <ul className="grid grid-cols-2 gap-1">
          {systemStatus.services.slice(0, 8).map((s) => (
            <li
              key={s.name}
              className="flex items-center gap-1.5 font-mono text-2xs text-ink-muted"
            >
              <span
                className={cn(
                  "led",
